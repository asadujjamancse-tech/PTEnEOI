...
export default function ReadingPanel() {
  ...
  const filtered = useMemo(() => READING_QS.filter(q => (!query || (q.title + q.text + q.tags.join(' ')).toLowerCase().includes(query.toLowerCase()))
  ...
  return (
    <div style={{ background: '#0F1929', border: '1px solid #1E293B', borderRadius: 12, padding: 16 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        <div>
          <div style={{ fontSize: 16, fontWeight: 700 }}>Reading Practice</div>
          <div style={{ color: '#64748B', fontSize: 13 }}>Timed readings with comprehension prompts.</div>
        </div>
        <input className="input-field" style={{ width: 240 }} placeholder="Search passages or tags..." value={query} onChange={e => setQuery(e.target.value)} />
      </div>

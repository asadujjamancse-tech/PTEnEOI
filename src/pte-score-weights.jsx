...
function SectionPanel({ section, active, onClick }) {
  const isActive = active === section.id;
  return (
    <button
      onClick={() => onClick(section.id)}
      style={{
        background: isActive ? section.bg : "#0A1222",
        border: `1px solid ${isActive ? section.color : "#1E293B"}`,
        borderRadius: 12,
        padding: "12px 18px",
        cursor: "pointer",
...
export default function PTEWeights() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#070D1A", color: "#fff", fontFamily: "'DM Sans', system-ui, sans-serif", padding: "28px 24px" }}>
      <style>{`
      ...
      <div style={{ maxWidth: 900, margin: "0 auto" }}>
        ...
        {/* Official note */}
        <div style={{ background: "#0C1B35", border: "1px solid #1D4ED8", borderRadius: 10, padding: "10px 16px", marginBottom: 24, fontSize: 12, color: "#60A5FA", lineHeight: 1.6 }}>
        ...
        <div style={{ display: "flex", gap: 10, marginBottom: 24, flexWrap: "wrap" }}>
          {SECTIONS.map(s => <SectionPanel key={s.id} section={s} active={active} onClick={handleTab} />)}
        </div>

        {/* View toggle */}
        <div style={{ display: "flex", gap: 8, marginBottom: 20, justifyContent: "flex-end" }}>
          {[["table", "📋 Table"], ["chart", "📊 Chart"], ["both", "⊞ Both"]].map(([v, l]) => (
            <button key={v} onClick={() => setView(v)} style={{ background: view === v ? section.color + "20" : "#0F1929", border: `1px solid ${view === v ? section.color : "#1E293B"}`, color: view === v ? section.color : "#64748B", borderRadius: 8, padding: "6px 14px", fontSize: 12, fontWeight: 600, cursor: "pointer", transition: "all .2s" }}>{l}</button>
          ))}
        </div>
        ...
        <div style={{ display: "flex", gap: 20, alignItems: "flex-start", flexWrap: "wrap" }}>
          ...
          {(view === "table" || view === "both") && (
            <div style={{ flex: view === "both" ? "1 1 420px" : "1 1 100%", background: "#0A1222", border: `1px solid ${section.border}`, borderRadius: 16, overflow: "hidden" }}>
              {/* Section header */}
                ...
                return (
                  <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 60px 90px 90px", padding: "12px 20px", borderBottom: i < section.tasks.length - 1 ? "1px solid #0F1929" : "none", background: isTop ? section.bg : "transparent", transition: "background .15s", alignItems: "center" }}>
                    ...
                    <div style={{ textAlign: "center" }}>
                      {task.feeds !== "—" ? (
                        <span style={{ fontSize: 11, background: "#1E293B", color: "#94A3B8", borderRadius: 6, padding: "2px 8px" }}>{task.feeds}</span>
                      ) : (
          ...
          {(view === "chart" || view === "both") && (
            <div style={{ flex: view === "both" ? "1 1 300px" : "1 1 100%", display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div style={{ background: "#0A1222", border: `1px solid ${section.border}`, borderRadius: 16, padding: "20px", overflow: "hidden" }}>
              ...
              {/* 80% insight card */}
              <div style={{ background: "#0C1B35", border: "1px solid #1D4ED8", borderRadius: 14, padding: "16px 18px" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {(() => {
                    ...
                    return section.tasks.filter(t => { const over = cum >= 80; cum += t.pct; return !over; }).map((t, i) => (
                      <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: "#0F1929", borderRadius: 8, padding: "8px 12px" }}>
              ...
              {section.tasks.some(t => t.feeds !== "—") && (
                <div style={{ background: "#1A1305", border: "1px solid #92400E", borderRadius: 14, padding: "14px 16px" }}>
        ...
        <div style={{ marginTop: 28, background: "#0A1222", border: "1px solid #1E293B", borderRadius: 16, overflow: "hidden" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 0 }}>
            {SECTIONS.map((s, si) => (
              <div key={s.id} style={{ borderRight: si < 3 ? "1px solid #1E293B" : "none", padding: "16px 16px" }}>
